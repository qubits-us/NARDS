...
type
  TMainFrm = class(TForm)
    ...
    procedure sckUDPDataAvailable(Sender: TObject; ErrCode: Word);
    procedure SendCommandID(aCommandID:Int32);
    procedure sckServerSessionConnected(Sender: TObject; ErrCode: Word);
    procedure sckServerSessionClosed(Sender: TObject; ErrCode: Word);
    procedure pnlTopGesture(Sender: TObject; const EventInfo: TGestureEventInfo; var Handled: Boolean);
...
var
  ...
  UDP_buff: array[0..99] of byte;
  UDP_recvCount:integer;
  SRV_buff:array[0..99] of byte;
  SRV_recvCount:integer;
...
procedure TMainFrm.SendCommandID(aCommandID: Integer);
var
aPack:tCommandIdPacket;
begin

  if sckServer.State = wsConnected then
   begin
    //
    FillPacketIdent(aPack.header.Ident);
    aPack.header.Command:=CMD_SETID;
    aPack.header.Option[0]:=0;
    aPack.header.Option[1]:=0;
    aPack.header.Option[2]:=0;
    aPack.header.Option[3]:=0;
    aPack.header.DataSize:=SizeOf(Int32);
    aPack.CommandID:=aCommandId;
    sckServer.Send(@aPack,SizeOf(aPack));
   end;
end;


//refreshes screen from server request..
procedure TMainFrm.sckServerSessionClosed(Sender: TObject; ErrCode: Word);
begin
mLog.Lines.Insert(0,'Disconnected from NARD server..');
  sbMain.Panels[2].Text := 'Server Offline..';

end;

procedure TMainFrm.sckServerSessionConnected(Sender: TObject; ErrCode: Word);
begin
mLog.Lines.Insert(0,'Connected to NARD server..');
sbMain.Panels[2].Text := 'Server Online..';
end;

procedure TMainFrm.sckUDPDataAvailable(Sender: TObject; ErrCode: Word);
var
Len:integer;
aPack:tPacketUDP;
begin
    Len := sckUDP.Receive(@UDP_buff[UDP_recvCount], SizeOf(UDP_buff)-UDP_recvCount);
    if Len <= 0 then
        Exit;
        UDP_recvCount:=UDP_recvCount +Len;
    if UDP_recvCount >= SizeOf(aPack) then
      begin
      move(UDP_buff,aPack,SizeOf(aPack));
        if CheckPacketIdent(aPack.Ident) then
     ...
     //how many nard are we laying out..
      while not dmDb.qryScreenItems.EOF do
       begin
        ...
        sbMain.Panels[1].Text := 'Nards:'+IntToStr(NumOnline);
...
sckServer.Addr:=ServerIp;
sckServer.Port:=ServerPort;
sckServer.BufSize:=100;
SRV_recvCount:=0;
sckServer.Connect;
...
procedure TMainFrm.pnlTopGesture(Sender: TObject; const EventInfo: TGestureEventInfo; var Handled: Boolean);
begin
mLog.Lines.Insert(0,IntToStr(EventInfo.GestureID));
end;
