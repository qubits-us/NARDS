...
public:
  ...
  typedef bool    (*BlobGet) (const uint8_t, uint8_t* , int32_t*);
  typedef bool    (*BlobSet) (const uint8_t, const uint8_t*, const int32_t, const uint16_t idNard);  
  ...
  void onBlobs(BlobGet getStr, BlobSet setStr);
  ...
  //set a blob
  bool setBlob(const uint8_t index, const uint8_t* buff, const int32_t size, uint16_t idNard = 0);
